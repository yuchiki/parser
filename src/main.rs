// サンプルの例だと全部mainに書いてある。めんどうなのでそれにならってしまう
use std::io;
...
fn prompt(s: &str) -> io::Result<()> {
    ...
    let stdout = stdout();
    let mut stdout = stdout.lock(); // ここ何してるかあんまわからんけどあとでいいか
...
fn main() {
    ...
    let stdin = stdin();
    let stdin = stdin.lock(); // ここメソッドチェーンで書いちゃいかんのかな
    let stdin = BufReader::new(stdin); // ここら辺メソッドチェーンでかけないのか？
        ...
        .and_then(|tok| match tok.value {
            ...
            TokenKind::LParen => {
                ...
                match tokens.next() {
                    ...
                    _ => Err(ParseError::UnclonsedParen(tok)),
                }
            }
